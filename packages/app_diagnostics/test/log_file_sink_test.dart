...
void main() {
  ...
  test('names the worker stream with the spelling that app already wrote', () {
    // A suffix is a name on the user's disk, not just a wire value: an app
    // updated mid-recording has to find the file its previous build wrote. So
    // this is `-worker` because that is what is out there, not `-wrk` because
    // that is shorter.
    final dir = Directory.systemTemp.createTempSync('sinknames');
    addTearDown(() => dir.deleteSync(recursive: true));

    expect(
      LogFileSink.fileFor(dir, 'abc', LogStream.worker).path,
      endsWith('session-abc-worker.jsonl'),
    );
  });
