...
/// An application's identifier grammar, in miniature: `id@VALUE`, where VALUE
/// has to be one of a closed list. The closed list is the point — an app puts
/// content in an identifier only where it can prove the content is not the
/// user's, and an unrecognised value has to leave no trace.
({String id, Map<String, Object?> fields}) splitOnAt(String rawId) {
  const known = {'ALPHA', 'BETA'};
  final at = rawId.indexOf('@');
  if (at < 0) return (id: rawId, fields: const {});
  final value = rawId.substring(at + 1);
  return (
    id: rawId.substring(0, at),
    fields: known.contains(value) ? {'tag': value} : const {},
  );
}

/// The same, written the way a call site would write it.
Widget tagWith(String id, String value, Widget child) =>
    logTag('$id@$value', child);
...
void main() {
  ...
  /// Swaps in a probe that knows the app grammar above. Called before
  /// `pumpApp`, which is what attaches it.
  void useDecomposer() {
    probe.detach();
    probe = InteractionProbe(store: store, decompose: splitOnAt);
  }
  ...
  testWidgets('stamps a touch with the moment the finger went down', (
    ...
    var now = origin;
    store = LogStore(
      header: LogHeader(ts: origin, session: 'test', app: '0.11.2+1102'),
      clock: () => now,
    );
    probe = InteractionProbe(store: store);
  ...
  testWidgets('records what the decomposer read out of an identifier', (
    tester,
  ) async {
    // The identifier is the only channel this probe has, so an application that
    // wants a fact about the control alongside the id has to encode it there —
    // and it comes back out split, never as part of the id.
    useDecomposer();
    await pumpApp(
      tester,
      Center(
        child: tagWith(
          'inventory.spool',
          'ALPHA',
          ElevatedButton(onPressed: () {}, child: const Text('Wyślij')),
        ),
      ),
    );
    ...
    expect(record['tag'], 'ALPHA');
  });

  testWidgets('a value the decomposer rejects leaves no trace', (tester) async {
    // Not in the field, and — the half that is easy to get wrong — not left
    // stuck on the end of the id either.
    useDecomposer();
    await pumpApp(
      tester,
      Center(
    ...
    final record = stop().single;
    expect(record['id'], 'inventory.spool');
    expect(record.containsKey('tag'), isFalse);
  });

  testWidgets('the decomposed fields reach a control inside the tagged card', (
    tester,
  ) async {
    // Same inheritance as the identifier: a tap on something deeper inside the
    // tile still knows what the tile said.
    useDecomposer();
    await pumpApp(
      tester,
      Center(
    ...
    final record = stop().single;
    expect(record['id'], 'inventory.spool');
    expect(record['tag'], 'BETA');
  });

  testWidgets('an identifier with no decomposer is the id, whole', (
    tester,
  ) async {
    // The default. An app that encodes nothing must not have its identifier
    // reinterpreted by a package that does not know its grammar.
    await pumpApp(
      tester,
      Center(
        child: logTag(
          'inventory.spool@ALPHA',
          ElevatedButton(onPressed: () {}, child: const Text('Wyślij')),
        ),
      ),
    );
    ...
    final record = stop().single;
    expect(record['id'], 'inventory.spool@ALPHA');
    expect(record.containsKey('tag'), isFalse);
  });
  ...
  testWidgets('a drag does not claim what the row under it said', (
    tester,
  ) async {
    // One drag record stands for a folded burst of flicks over different rows,
    // so a value read off the row under the *first* finger-down would speak for
    // rows it never touched. The rule is about where the value came from, so it
    // drops everything the decomposer returned rather than one known key.
    useDecomposer();
    await pumpApp(
      tester,
      ListView(
    ...
    expect(drag.containsKey('tag'), isFalse);
  });
