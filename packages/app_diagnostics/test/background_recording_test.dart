...
void main() {
  ...
  group('refuses to record', () {
    ...
    test('when the session already ran out of its five minutes', () async {
      await sessions.saveSession(session);
      await writeUiStream();
      now = sessionStart.add(testSessionLimit).add(const Duration(seconds: 1));
  ...
  group('continues the session the UI started', () {
    ...
    test('the deadline belongs to the session, not to the store', () async {
      ...
      expect(last['minutes'], testSessionLimit.inMinutes);
    });
    ...
    test('flushes the application\'s probes as it closes', () async {
      // The isolate's own teardown runs first and may still be writing records
      // through the static, so this is the last thing that happens — and it has
      // to happen here too, not only on the UI recorder, or the background
      // stream loses its tail in exactly the isolate that needed the mirror.
      await sessions.saveSession(session);
      await writeUiStream();
