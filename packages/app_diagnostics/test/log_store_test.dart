...
void main() {
  ...
  LogStore makeStore({
    ...
    header: LogHeader(ts: start, session: 'test-session', app: '0.11.2+1102'),
    redactor: redactor,
  ...
  test('records are redacted on the way in', () {
    final redactor = testRedactor()..remember('my-secret-key', '[APIKEY]');
  ...
  test('a session ends itself at the size ceiling and says which one', () {
    ...
    final store = LogStore(
      header: LogHeader(ts: start, session: 'test-session', app: '0.11.3+1103'),
  ...
  test('a session ends itself at the time ceiling', () {
    ...
    store.add(LogSource.ui, 'tap');
    now = start.add(testSessionLimit).add(const Duration(seconds: 1));
    store.add(LogSource.ui, 'tap');
    store.add(LogSource.ui, 'tap');
    ...
    expect(records.last['minutes'], testSessionLimit.inMinutes);
  ...
  test('a store handed the session start inherits its deadline', () {
    ...
    now = start.add(testSessionLimit - const Duration(seconds: 30));
    final store = LogStore(
      header: LogHeader(
        ts: start,
