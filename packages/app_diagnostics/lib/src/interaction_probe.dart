...
class InteractionProbe {
  InteractionProbe({
    required this.store,
    this.decompose = noSemanticsIdFields,
  ...
  /// Splits a raw `Semantics(identifier:)` into the id the log records and any
  /// fields the application encoded alongside it. See [SemanticsIdDecomposer].
  final SemanticsIdDecomposer decompose;
  ...
  TouchTarget? _targetAt(Offset position, int viewId) {
    ...
    return TouchTarget.of(
      node,
      inheritedId: hit.id ?? _menuItemId(node),
      decompose: decompose,
    );
  }
...
/// Splits a raw `Semantics(identifier:)` into the id the log records and
/// whatever else the application chose to encode in it.
///
/// It has to do both halves in one step. An application that writes
/// `inventory.spool@PETG` means "the control is `inventory.spool`, and it was
/// showing PETG" — a hook that only *added* a field would leave the whole
/// string standing as the id, which is a different value in the log and a
/// different thing to group by.
///
/// Whatever comes back in `fields` is content, not structure, so a drag drops
/// it: see [TouchTarget.toFields].
typedef SemanticsIdDecomposer =
    ({String id, Map<String, Object?> fields}) Function(String rawId);

/// The default: the identifier is the id, and nothing rides along with it.
({String id, Map<String, Object?> fields}) noSemanticsIdFields(String rawId) =>
    (id: rawId, fields: const {});

/// What we learned about the thing under the finger. Accessibility labels are
/// **not** part of this on purpose; [fields] is the single exception, and only
/// for what the application's own decomposer put in an identifier.
class TouchTarget {
  const TouchTarget({
    this.id,
    this.fields = const {},
  ...
  factory TouchTarget.of(
    SemanticsNode node, {
    String? inheritedId,
    SemanticsIdDecomposer decompose = noSemanticsIdFields,
  }) {
    final data = node.getSemanticsData();
    final flags = data.flagsCollection;
    final identifier = data.identifier.isEmpty ? inheritedId : data.identifier;
    final tag = identifier == null ? null : decompose(identifier);
    return TouchTarget(
      id: tag?.id,
      fields: tag?.fields ?? const {},
      role: _roleOf(data),
  ...
  /// What the application's [SemanticsIdDecomposer] read out of the
  /// identifier. Empty unless one was supplied.
  ...
  /// [forDrag] drops what only means something for a press: everything the
  /// decomposer read out of the identifier, and `empty`.
  ///
  /// A drag record stands for a folded burst of flicks over different rows, so
  /// content read off the row under the *first* finger-down would speak for
  /// rows nobody touched. That is why the whole of [fields] goes, not one
  /// key of it: the rule is about where the value came from, and a decomposer
  /// this package cannot see is free to return a second one tomorrow.
  ///
  /// `empty` says nothing about a drag either, since the scrollable ancestor
  /// handles it whatever was underneath — a live run had one dashboard flick
  /// reported once as `empty` and once as a `button`. It is written only when
  /// true, or every line would carry a field saying "this one did hit
  /// something".
