...
/// Fallback session ceilings, deliberately generous.
///
/// The real ones belong to the application: how long a reproduction takes and
/// how much log that is worth are answers about *that* app's bugs, and the two
/// on this package cannot be argued from anything. They exist so a caller that
/// has not decided yet still gets a bounded session rather than an unbounded
/// one — never as a recommendation.
const defaultSessionDuration = Duration(minutes: 30);
const defaultSessionBytes = 20 * 1024 * 1024;
...
class LogStore {
  LogStore({
    ...
    this.maxDuration = defaultSessionDuration,
    this.maxBytes = defaultSessionBytes,
    DateTime Function()? clock,
    DateTime? openedAt,
    this.onLine,
    this.onClosed,
  }) : redactor = redactor ?? LogRedactor(ourKeys: const {}),
       _openedAtOverride = openedAt,
       _clock = clock ?? (() => ambient.clock.now());

  final LogHeader header;

  /// Defaulted only so a store can be built without one — which in practice
  /// means a test. The fallback has an empty `ourKeys`, so it scrubs *more*
  /// than a configured one rather than less: a store nobody configured cannot
  /// leak, it can only over-redact. The application's real vocabulary comes in
  /// through `DiagnosticRecorder.redactor`, which is required.
  ...
  /// in a backgrounded app fires late or not at all, and a background isolate
  /// keeps logging into its own store the whole time.
  ...
  /// Mirrors each encoded line to a durable sink. A background isolate needs it
