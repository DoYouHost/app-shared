...
/// handling exactly as it was. [HttpProbe], [NavigationProbe] and whatever the
/// application registers as a [DiagnosticSessionListener] live for the app's
/// lifetime and read [active] per event.
///
...
class DiagnosticRecorder {
  DiagnosticRecorder({
    ...
    this.sessionBytes = defaultSessionBytes,
  ...
  final DiagnosticsSessionStore sessions;
  final Future<SessionFacts> Function() loadFacts;

  /// Builds the session's redactor. A factory rather than an instance: each
  /// session remembers its own secrets, and one shared across sessions would
  /// carry the previous server's host into the next recording.
  final LogRedactor Function() redactor;

  /// The session's two hard ceilings, which belong to the application — how
  /// long its bugs take to reproduce, and how much log that is worth.
  final Duration sessionDuration;
  final int sessionBytes;

  /// Told when a session opens and when it closes; see
  /// [DiagnosticSessionListener].
  final List<DiagnosticSessionListener> listeners;
  ...
  Future<void> start({void Function(String limit)? onLimitReached}) async {
    ...
    final sessionRedactor = redactor();
    facts.secrets.forEach(sessionRedactor.remember);
  ...
  static Future<BackgroundRecording?> startBackground({
    required DiagnosticsSessionStore sessions,
    required LogStream stream,
    required LogRedactor Function() redactor,
    required Duration sessionLimit,
    Future<Directory?> Function() resolveDirectory = diagnosticsDirectory,
    Future<Map<String, String>> Function()? loadSecrets,
    List<DiagnosticSessionListener> listeners = const [],
    ...
    try {
      ...
      if (elapsed.isNegative || elapsed >= sessionLimit) return null;

      final sessionRedactor = redactor();
      (await loadSecrets?.call() ?? const <String, String>{}).forEach(
        sessionRedactor.remember,
      );
      ...
      final store = LogStore(
        ...
        maxDuration: sessionLimit,
        clock: clock,
        onLine: sink.writeLine,
      );
      _active = store;
      final errors = attachErrors ? (ErrorProbe(store: store)..attach()) : null;
      return BackgroundRecording._(store, sink, errors, listeners);
    } on Object {
...
class BackgroundRecording {
  ...
  final List<DiagnosticSessionListener> _listeners;

  /// Closes the stream: listeners flushed, global error handlers restored,
  /// queued lines flushed to disk.
  ///
  /// Call it **after** the isolate's own teardown. A client's `dispose` writes
  /// its last records through the static, so a stream closed first would lose
  /// the end of that story; the flush here is then a no-op and stays for the
  /// case where no client was ever built.
  ///
