...
/// the fields that read numbers carry values where both readings are
/// plausible — a weight in grams, a four-figure cost, an odometer. "0,500" is
/// not ambiguous at all, since nothing is grouped below a thousand. There is no
/// locale to ask here, and of the two ways to be wrong, refusing shows the user
/// a field they can correct while guessing stores a number nobody typed. Any
/// other comma is a decimal point.
///
/// Blank or null text answers null as well. A field means "unset" by an empty
/// box, and each one already decides for itself whether that is a value or a
/// mistake, so telling the two apart is the caller's business.
...
///
/// There "1000.5" is accepted on screen, and refusing it here would drop a
/// value the user typed and watched pass. Only for a field validated that way —
/// see [parseUserInt] for why it is not the default.
