...
///
/// The method-channel wrappers this replaced had different policies for the
/// same question — what happens when the channel is not there — and the
/// difference was accidental rather than argued: one caught both channel
/// failures, one caught both and fell back to a default, and one caught neither
/// and guarded on `Platform.isAndroid` instead. All of them meant the same
/// thing by it: this host does not implement the feature, so the caller gets
/// the answer a device without it would give.
class PlatformQuery {
  ...
  /// different screen for each, talks to [channel] itself.
  Future<T> ask<T>(
    String method, {
    required T fallback,
