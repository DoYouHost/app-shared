...
void main() {
  group('header', () {
    ...
    test(
      'passes through what the relay would refuse, rather than repairing it',
      () {
  ...
  group('schema', () {
    ...
    test(
      'reports an older recording under its own version, not this build',
      () {
    ...
    test(
      'is whatever the application passed, not a number this package owns',
      () {
        ...
        expect(
          reportEnvelope(logWith({'app': '0.9'}), formatVersion: 4).logSchema,
          4,
        );
      },
    );
  ...
  group('a request, which has no log to read a header off', () {
    test('carries the versions and the language, and nothing else', () {
      ...
      expect(
        ...
        unorderedEquals(['v', 'ts', 'app', 'server', 'locale']),
      );
    });
    ...
    test('only the bug kind carries a recording', () {
      ...
      expect(ReportKind.values.map((k) => k.name), [
        'bug',
        'change',
        'feature',
      ]);
