...
class RelayClient {
  RelayClient(this._dio, {required this.baseUrl});

  final Dio _dio;

  /// Where reports go, up to but not including the endpoint. Public by
  /// necessity — it ships inside the APK — which is why the relay meters every
  /// caller rather than trusting this to stay unknown.
  ///
  /// The path prefix names the application: one relay serves several, and it
  /// decides which repository a report reaches from the prefix the challenge was
  /// issued under. Both endpoints hang off this, so it must not end in a slash.
  ///
  /// Required rather than defaulted: a package that shipped one application's
  /// URL as the fallback would let the next application file its reports into
  /// the wrong repository by simply forgetting to pass one.
