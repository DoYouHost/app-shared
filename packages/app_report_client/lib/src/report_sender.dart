...
class ReportSender {
  ReportSender({
    ...
    required this._formatVersion,
  });

  final RelayClient _client;
  final ReportOutbox _outbox;

  /// The log schema this build writes. Held rather than passed per call because
  /// [submit] derives the envelope from the log itself and so has nobody to ask.
  ...
  Future<void> submit({
    ...
    required String log,
      ...
      _commit(
        ...
        envelope: reportEnvelope(log, formatVersion: _formatVersion),
        log: log,
      );
