...
void main() {
  Future<ProviderContainer> pumpScreen(WidgetTester tester, {Rig? rig}) async {
    final container = (rig ?? Rig()).container();
  ...
  testWidgets('says what the log will contain, in the application\'s words', (
    tester,
  ) async {
    // This is what the user agrees to before anything is recorded, and only the
    // application knows which of its probes are wired up — so the lines are its
    // own, and the card around them is this package's.
    await pumpScreen(tester);

    expect(find.text(consent.recorded.last), findsOneWidget);
    // Both halves of the promise: what is kept, and what never goes in.
    expect(find.text(consent.neverRecorded.first), findsOneWidget);
    expect(find.text(consent.neverRecorded.last), findsOneWidget);
  ...
  /// The rig decides where "back to the user" is: a configured app has a
  /// dashboard, a fresh one only has setup.
  Future<ProviderContainer> pumpRouted(WidgetTester tester, {Rig? rig}) async {
    final container = (rig ?? Rig()).container();
  ...
  testWidgets('starting hands the app straight back to the user', (
    ...
    // The bug is reproduced back in the app, so that is where the user is put.
  ...
  testWidgets('a recording can start before the server is set up', (
    ...
    final container = await pumpRouted(tester, rig: Rig(withProfile: false));
  ...
  group('handing the log over', () {
    ...
    Future<ProviderContainer> pumpReview(
      ...
      final container = await pumpRouted(tester, rig: rig);
      await container.read(bugReportProvider.notifier).start();
    ...
    testWidgets('saving it names a text file and then cleans up', (
      ...
      expect(name, startsWith('app-log-'));
    ...
    testWidgets('opens on keeping the log, not on publishing it', (
      ...
      await pumpReview(tester, rig: Rig(relay: relay));
    ...
    testWidgets('backing out of the picker keeps the report', (tester) async {
      final container = await pumpReview(
        tester,
        rig: Rig(saver: fakeSaver(LogSaveResult.cancelled)),
      );
    ...
    testWidgets('a save that fails says so and keeps the report', (
      ...
      final container = await pumpReview(
        tester,
        rig: Rig(saver: fakeSaver(LogSaveResult.failed)),
      );
  ...
  testWidgets('a session id left over from a crash does not fake a recording', (
    ...
    final rig = Rig(orphanSession: 'abc123');
    final container = await pumpScreen(tester, rig: rig);
    ...
    expect(rig.sessions.loadSession(), isNull);
  });
  ...
  group('after a crash', () {
    ...
    const header =
        ...
        '"stream":"ui","app":"0.11.2+1102"}';

    setUp(() {
      dir = Directory.systemTemp.createTempSync('report_recover');
    ...
    /// the store and whatever the mirror had already flushed to disk.
    ///
  ...
  group('recording bar', () {
    ...
    final clockText = '0:00 / ${formatElapsed(sessionDuration)}';
    ...
    testWidgets('ends itself at the limit and says so', (tester) async {
      ...
      expect(
        find.textContaining('minął limit ${sessionDuration.inMinutes} min'),
        findsOneWidget,
      );
  ...
  group('changes and features', () {
    Future<ProviderContainer> pumpIdle(WidgetTester tester, {Rig? rig}) =>
        pumpRouted(tester, rig: rig);
    ...
    testWidgets('the change wording is not the feature wording', (
      ...
      await pumpIdle(
        tester,
        rig: Rig(relay: FakeRelay(issued: ticket())),
      );
    ...
    testWidgets('facts it cannot read say so instead of hanging the button', (
      ...
      final container = await pumpIdle(
        ...
        rig: Rig(
          relay: relay,
          outboxRoot: root,
          loadFacts: () => Future.error(StateError('no package info')),
        ),
      );
