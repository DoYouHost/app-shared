...
/// gets reproduced elsewhere in the app, not on the report screen. Also the
/// visible indicator that a recording is running: nobody should be able to
/// leave one going without noticing.
class RecordingBannerScaffold extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen(bugReportProvider, (previous, next) {
      ...
      if (limit != null) _announceLimit(context, ref, limit);
    });
  ...
  void _announceLimit(BuildContext context, WidgetRef ref, String limit) {
    ...
    ScaffoldMessenger.of(context).replaceSnack(
      ...
      action: SnackBarAction(
        ...
        onPressed: () {
          final navigator = bindings.navigatorKey.currentContext;
...
class _RecordingLayerState extends ConsumerState<_RecordingLayer> {
  ...
  /// `0:42 / 30:00`. The ceiling is part of the clock because the recording
  /// ends on it by itself — a bar that simply vanished would read as a
  /// malfunction.
  String _elapsed(DateTime? startedAt) {
    final elapsed = startedAt == null
        ? Duration.zero
        : DateTime.now().difference(startedAt);
    final limit = ref.read(reportBindingsProvider).recorder.sessionDuration;
    return '${formatElapsed(elapsed)} / ${formatElapsed(limit)}';
  }
  ...
  Widget _shell(BuildContext context, Widget child) {
    ...
    return Semantics(
      ...
      label: ReportLocalizations.of(context).bugReportBannerLabel,
      child: Material(
        key: _barKey,
