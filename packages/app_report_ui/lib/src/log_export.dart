...
/// app's history. A long recording is far past both.
///
...
/// Overridable so a widget test can drive the save without a real dialog —
/// the picker is a platform channel with no test double.
final logFileSaverProvider = Provider<LogFileSaver>((_) => saveLogFile);

/// A session's log is text — a few megabytes at worst — so the system save
/// dialog is the right hand-off.
@visibleForTesting
...
String logFileName(String prefix, DateTime at) {
  String two(int value) => value.toString().padLeft(2, '0');
  return '$prefix-log-${at.year}${two(at.month)}${two(at.day)}'
