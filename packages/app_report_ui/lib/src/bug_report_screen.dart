    ...
    return DashBackground(
      child: Scaffold(
        ...
        body: MaxContentWidth(
          maxWidth: ref.watch(reportBindingsProvider).maxContentWidth,
...
class _IdleViewState extends ConsumerState<_IdleView> {
  ...
  Widget build(BuildContext context) {
    ...
    final consent = ref.read(reportBindingsProvider).consent(context);
    ...
    return ListView(
      ...
      children: [
        ...
        if (state.kind.needsLog) ...[
          // A report queued earlier — a request a moment ago, or anything the
          // last run left in the outbox — keeps counting down whatever tab is
          // open. Shown here too, or the countdown would run on a screen that
          // says nothing about it and the one control that can still stop it
          // would be a tab away.
          ...
          ..._bugSteps(l10n, consent),
        ] else
          ..._requestForm(l10n, state, consent),
      ],
    );
  }

  List<Widget> _bugSteps(ReportLocalizations l10n, ReportConsent consent) => [
    _Card(
    ...
    _Card(
      ...
      rows: [
        for (final line in consent.recorded) _Fact.kept(line),
        for (final line in consent.neverRecorded) _Fact.never(line),
      ],
    ...
    return [
      ...
      _Card(
        ...
        rows: [
          ...
          _Fact.never(consent.requestExcludes),
        ],
  ...
  /// from a menu and the recording bar pushes it again when the user finishes.
      ...
      children: [
        _Card(
          ...
          body:
              ...
              '${l10n.bugReportLimit(ref.read(reportBindingsProvider).recorder.sessionDuration.inMinutes)}',
  ...
  Future<void> _save() async {
    ...
    final result = await ref.read(logFileSaverProvider)(
      fileName: logFileName(
        ref.read(reportBindingsProvider).logFilePrefix,
        DateTime.now(),
      ),
...
class _Waiting extends StatelessWidget {
  ...
  /// without it, a bug report still waiting reads as the request being typed
  /// having been sent already.
  final ReportKind? otherKind;

  static String _queuedText(ReportLocalizations l10n, ReportKind kind) =>
      switch (kind) {
...
class _Failed extends StatelessWidget {
  ...
  static String _failureText(ReportLocalizations l10n, RelayFailure? failure) =>
      switch (failure) {
...
class _CancelSend extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) => TextButton(
    ...
    child: Text(ReportLocalizations.of(context).bugReportCancelSend),
        ...
        children: [
          ...
          if (summary.markers > 0) ...[
            ...
            Text(
              ...
              style: t.labelSoft.copyWith(color: t.accentInk),
            ),
          ],
          ...
          if (sessionFacts.isNotEmpty) ...[
            ...
            Text(
              ...
              [
                for (final e in sessionFacts.entries) '${e.key} ${e.value}',
              ].join('  ·  '),
              style: t.monoMicro.copyWith(height: 1.5),
            ),
          ],
        ],
      ),
        ...
        children: [
          Padding(
            ...
            child: Icon(
              ...
              size: 15,
              color: included ? t.accentInk : t.textTertiary,
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
